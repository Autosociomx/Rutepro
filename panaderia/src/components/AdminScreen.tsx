...
import { Product, Seller, Venta, VentaItem, AppConfig, Devolucion, MysteryAudit, Abono, Client, ChatMsg } from '../types';
...
export const AdminScreen: React.FC<AdminScreenProps> = ({ cfg, onGoBack, triggerToast, onGoConfig, onCerrarSesion }) => {
  ...
  const getRouteBreadcrumbs = (sellerId: string) => {
    ...
    if (mappedStops.length === 0) {
      const fallbackClientNames = [
        ...
        { name: 'MiniSúper Express Gaby', tipo: 'Minisuper', tip: 'Revisar mermas de pan y resurtir inventario.', x: 300, y: 140 },
  ...
  const handleAskAI = async (textOver?: string) => {
    ...
    const newLogs: ChatMsg[] = [...chatLogs, { role: 'usr', text: qStr }];
  ...
  const startVoiceInput = () => {
    ...
    if (!SpeechRecognition) {
      triggerToast('Tu navegador no soporta dictado por voz.', 'err');
      return;
    }
    ...
    recognition.onstart = () => {
      setIsListening(true);
      triggerToast('Escuchando... Habla ahora en español.', 'ok');
    };

    recognition.onresult = (event: any) => {
      const speechToText = event.results[0][0].transcript;
      setChatInp(speechToText);
      setIsListening(false);
      triggerToast('Petición capturada: ' + speechToText);
    };

    recognition.onerror = (event: any) => {
      console.error('Speech recognition error:', event.error);
      setIsListening(false);
      triggerToast('Error en dictado: ' + event.error, 'err');
    };
        ...
        <div className="bg-[#0B0E14] border-b border-white/5 p-4.5 space-y-3 animate-fade-in absolute top-14 left-0 right-0 z-40 shadow-2xl">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <button 
              ...
              className="p-3 bg-[#111520] border border-white/5 rounded-xl hover:bg-[#181D2B] text-left text-xs text-amber-300 cursor-pointer transition-all font-bold"
            >
              ⚙️ Ajustes Catálogo
            </button>
            <button 
      ...
      <div className="flex-1 max-w-lg mx-auto w-full px-4 pt-4 space-y-4">
        ...
        >
          ...
          <div className="shrink-0 space-y-1 bg-[#151925]/30 p-2 border-t border-white/5">
            ...
            {/* Input form bar */}
            <div className="flex gap-1.5 bg-[#0B0E14] border border-white/5 rounded-xl p-1.5 items-center">
              <button 
              ...
              <input 
                ...
                placeholder={isListening ? "Escuchando en español..." : "Escribe tu consulta..."}
              />
              <button 
                onClick={() => handleAskAI()} 
